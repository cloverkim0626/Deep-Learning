...
import { CheckCircle, Clock, AlertTriangle, ChevronDown, ChevronUp, RefreshCw, Trash2 } from "lucide-react";
    ...
    <div className="p-6 md:p-12 pb-20 max-w-4xl mx-auto overflow-y-auto custom-scrollbar h-full">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-4">
        <div>
          ...
          <p className="text-[14px] text-accent mt-2 font-medium">학생 클리닉 접수 현황 (15초마다 자동 갱신)</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        {[
          ...
          { label: "완료", count: queue.filter(q => q.status === "completed").length, color: "text-success" },
        ].map(s => (
          <div key={s.label} className="glass rounded-2xl p-5 border border-foreground/5 text-center">
            <div className={`text-2xl font-black ${s.color}`}>{s.count}</div>
            <div className="text-[11px] font-bold text-accent mt-1">{s.label}</div>
          </div>
        ))}
      </div>
            ...
            <div key={item.id}
              ...
              <button className="w-full flex items-center justify-between p-6 text-left gap-4"
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[12px] text-accent mt-0.5 truncate max-w-[200px] md:max-w-sm">
                      {item.topic}
                    </p>
                  </div>
                </div>
              ...
              {expandedId === item.id && (
                <div className="px-6 pb-6 border-t border-foreground/5 pt-5 space-y-5">
                  ...
                  <div>
                    <p className="text-[10px] font-bold text-accent uppercase tracking-widest mb-3">접수 시각</p>
